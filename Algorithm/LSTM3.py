...
def load_data(filename):
    ...
    x = data.drop(['成品率'], axis=1).drop(['卷号'],axis=1).drop(['时间'],axis=1)  # (*, 28)
    y = data['成品率']
    return x, y


train_x, train_y = load_data('../Datasets/成品率预测数据集--数字化后.xlsx')


BATCH_START = 0  # 建立 batch data 时候的 index
TIME_STEPS = 100  # backpropagation through time 的 time_steps
BATCH_SIZE = 100
INPUT_SIZE = 28  # sin 数据输入 size
OUTPUT_SIZE = 1  # cos 数据输出 size
CELL_SIZE = 300  # RNN 的 hidden unit size
...
if __name__ == '__main__':
    ...
    # print('实际', train_y.values.flatten().shape)
    ...
    line3, = axes.plot(range(100), train_y.values.flatten()[- 100:], 'r', label='true')
